 ...
 * «0/» prefix may come out as «O/» or be missed entirely, spacing wanders, and
...
export function parseMissionText(raw: string): MissionParseResult {
  ...
  for (const line of merged) {
    ...
    if (collect) {
      ...
      if (last && !last.origin) last.origin = collect[2].trim();
      continue;
    }

    if (isDeliverLike(line)) ignored.push(line);
  }
