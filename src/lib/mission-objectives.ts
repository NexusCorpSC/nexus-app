/**
 * Reading of the in-game mission log, as recognised from a screen capture.
 * Mirrors `lib/mission-objectives.ts` in nexus-tools.
 *
 ...
 *
 * This is the template the region capture is matched against: text that reads
 * like a mission log goes to the cargo sheet instead of the search palette.
 */
...
/*
 * OCR is not perfect on the in-game font, so the patterns stay forgiving: the
 * «0/» prefix may come out as «O/» or be missed entirely, spacing wanders, the
 * trailing period is optional, digits come back as the letters they look like,
 * and even the «Deliver» verb is sometimes lost — the «N SCU of … to …» shape
 * carries the objective on its own.
 */
...
/**
 * Where an objective starts. The verb opens one; so does a bare «0/3 SCU»,
 * but only when it is not the quantity of a «Deliver» just read.
 */
...
function splitObjectives(line: string): string[] {
  ...
  for (const match of line.matchAll(OBJECTIVE_START)) {
    ...
    if (!match[1]) {
      ...
      if (/deliver[\s,;:]*$/i.test(before)) continue;
    }
...
function isObjectiveLike(line: string): boolean {
  return /scu/i.test(line) && (/deliver/i.test(line) || OPENS_OBJECTIVE.test(line));
}
...
export function parseMissionText(raw: string): MissionParseResult {
  ...
  for (const segment of merged.flatMap(splitObjectives)) {
    ...
    if (isObjectiveLike(segment)) ignored.push(segment);
  }
...
/**
 * Reads pasted text, which may be either format: the usual
 * `Destination;Contenu;Volume;Emplacement` rows, or mission log text straight
 * from the game — or from a capture.
 */
